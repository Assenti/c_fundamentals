...
//Принцип заключается в том, что мы сразу находим самый меньший(больший) элемент в массиве
//и ставим его в начало массива, затем на каждой итерации проделывается то же самое, таким образом сортируется массив
int main()
{
	srand(time(0));
	int const size = 10;
