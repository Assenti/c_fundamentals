...
const int dna_size = 1000; //1000 - for good PC performance
...
int levenstein(char* value1, char*  value2);
...
int levenstein(char* value1, char*  value2) {
	int height = strlen(value1) + 1;
	int length = strlen(value2) + 1;
...
void patients_with_max_compatibilty_by_DNA_elems()
{
	...
	for (int i = 0; i <= total_patients_number; i++)
	{
		...
		{
			difference = levenstein(patient_records[i].dna_sequence, patient_records[j].dna_sequence);
			if (difference < min)
			{
				min = difference;
			}
			...
			/*cout << dna_elems_compare_results[i].patient_name << " and " << dna_elems_compare_results[j].patient_name << "\tDifference value: " <<
				difference << endl;*/
	...
	double Citosin_percent, double Timin_percent)
{
	...
	cout << "Do you want to display DNA sequence? (0 - no, yes - 1)" << endl;
	int doctors_choice;
	cin >> doctors_choice;
