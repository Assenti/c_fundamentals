...
void write_data_to_file(int current_index);
...
int main()
{
	...
	cout << "Choose the command code:" << endl;
	cout << "1 - Make a new transaction (with writing to file)" << endl;
	cout << "2 - Read data from file and display to console" << endl;
	int command;
	cin >> command;
...
void make_transaction(int sender_id, int recipient_id, string currency, double transaction_sum)
{
	...
	write_data_to_file(current_transaction_id);
...
ostream& operator << (ostream& os, const Bank_transaction& transaction)
{
	...
	os << transaction.sender.client_name << "\n" <<
...
void display_from_file()
{
	...
	cout << "Sender: " << temp.sender.client_name <<
		"\nRecipient: " << temp.recipient.client_name <<
		"\nTransaction #: " << temp.transaction_id <<
		"\nTransaction currency: " << temp.currency <<
		"\nTransaction amount: " << temp.transaction_sum <<
		"\nComission: " << temp.bank_comission << endl;
